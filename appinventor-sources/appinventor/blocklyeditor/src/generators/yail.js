...
Blockly.Yail.JAVA_ADD_COMPONENT = "private ";
...
Blockly.Yail.YAIL_COMMENT_MAJORt = "public class ";
...
Blockly.Yail.JAVA_SET_PROPERTY = "protected void $define()";
Blockly.Yail.JAVA_NEW_LINE = "\n";
Blockly.Yail.JAVA_OPEN_BRACKET = "{";
Blockly.Yail.JAVA_QUOTES = "\"";
Blockly.Yail.JAVA_TAB = "\t";
Blockly.Yail.JAVA_2TAB = "\t\t";
Blockly.Yail.JAVA_DISPATCH_PRE = "public boolean dispatchEvent(Component component, String componentName, String eventName, Object[] params){";
Blockly.Yail.JAVA_COMP_IF = "\t\t\tif (component.equals(";
Blockly.Yail.JAVA_EVENT_IF = ") && eventName.equals(\"";
Blockly.Yail.JAVA_IF_END = "\")) \n\t\t\t{\n";
Blockly.Yail.JAVA_ED = "\t\tEventDispatcher.registerEventForDelegation(this,\"";
...
Blockly.Yail.getFormYail = function(formJson, packageName, forRepl) {
  ...
  if (jsonObject.Properties) {
    ...
    formComponents = formProperties.$Components;
  ...
  //java imports
  if (!forRepl) {
    code.push(Blockly.Yail.writeJavaImports(packageName, mch));
  }
  ...
  //All top variables
  code.push(Blockly.Yail.writeJavaVariables(formName, mch, globalBlocks));
  
  //define method
  var tops = Blockly.mainWorkspace.getTopBlocks(true);
  code.push(Blockly.Yail.writeJavaDefine(mch, tops));
  
  //dispatchEvent header
  code.push(Blockly.Yail.JAVA_NEW_LINE + Blockly.Yail.JAVA_DISPATCH_PRE + Blockly.Yail.JAVA_NEW_LINE);
  code.push(Blockly.Yail.writeDispatchEvent(tops));
...
Blockly.Yail.writeJavaImports = function(packageName, mch) {
  var buff1;
  var buff2;
  buff1 = "package org." + packageName + ";"
   + "\n"
   + "\n"
     + "import "
     ...
     + "\n";
   buff2 = "";
   for (var i=0, comp; comp = mch[i]; i++){
      if(buff2.indexOf(comp.$Type)<0){
        buff2 += "import com.google.appinventor.components.runtime." + comp.$Type + ";";
        buff2 += "\n";
      }
      if(comp.$Components){
        for (var j = 0, comp2; comp2 = comp.$Components[j]; j++){
          if(buff2.indexOf(comp2.$Type)<0){
            buff2 += "import com.google.appinventor.components.runtime." + comp2.$Type + ";";
            buff2 += "\n";
          }
        }
      }
...
Blockly.Yail.writeJavaVariables = function(formName, mch, globalBlocks) {
  var buff1;
  var buff2;
  var buff3;
  buff1 = Blockly.Yail.YAIL_COMMENT_MAJORt + formName 
    + " extends Form implements HandlesEventDispatching {" 
    + Blockly.Yail.JAVA_NEW_LINE + Blockly.Yail.JAVA_NEW_LINE;
  buff2  = "";
  for (var i = 0, block; block = globalBlocks[i]; i++) {
    ...
    if (block.type == 'global_declaration') {
      buff2 += Blockly.Yail.JAVA_TAB;
      buff2 += "int " + temp[2] + " = " + temp[4] + ";";
      buff2 += Blockly.Yail.JAVA_NEW_LINE;
    }
  } 
  buff3 = "";
  for (var ii=0, comp; comp = mch[ii]; ii++) {
    buff3 += Blockly.Yail.JAVA_TAB + Blockly.Yail.JAVA_ADD_COMPONENT + comp.$Type + Blockly.Yail.YAIL_SPACER 
    + comp.$Name + Blockly.Yail.JAVA_SEMI + Blockly.Yail.JAVA_NEW_LINE;
    if(comp.$Components){
      for (var j = 0, comp2; comp2 = comp.$Components[j]; j++){
        buff3 += Blockly.Yail.JAVA_TAB + Blockly.Yail.JAVA_ADD_COMPONENT + comp2.$Type + Blockly.Yail.YAIL_SPACER 
        + comp2.$Name + Blockly.Yail.JAVA_SEMI + Blockly.Yail.JAVA_NEW_LINE;
      }
    }
  }

  return buff1+buff2+buff3;
};

Blockly.Yail.writeJavaDefine = function (mch, tops) {
  var buff1;
  var buff2;
  var buff3;

  buff1 = Blockly.Yail.JAVA_TAB + Blockly.Yail.JAVA_SET_PROPERTY + " " + Blockly.Yail.JAVA_OPEN_BRACKET 
    + Blockly.Yail.JAVA_NEW_LINE + Blockly.Yail.JAVA_NEW_LINE;
  
  buff2 = "";
  for (var i=0, comp; comp = mch[i]; i++) {
    buff2 += Blockly.Yail.JAVA_2TAB + comp.$Name + " = new " + comp.$Type + "(this);" + Blockly.Yail.JAVA_NEW_LINE 
    + Blockly.Yail.JAVA_NEW_LINE;
    for (var prop in comp) {
      if (prop.charAt(0) != "$" && prop != "Uuid") {
        buff2 += Blockly.Yail.JAVA_2TAB + comp.$Name + "." + prop + "( \"" + comp[prop] + "\" );" + Blockly.Yail.JAVA_NEW_LINE;
      }
    }
    buff2 += Blockly.Yail.JAVA_NEW_LINE + Blockly.Yail.JAVA_NEW_LINE;
    if(comp.$Components){
      for (var j = 0, comp2; comp2 = comp.$Components[j]; j++){
        buff2 += Blockly.Yail.JAVA_2TAB + comp2.$Name + " = new " + comp2.$Type + "(" + comp.$Name + ");" + Blockly.Yail.JAVA_NEW_LINE 
        + Blockly.Yail.JAVA_NEW_LINE;
        for (var prop in comp2) {
          if (prop.charAt(0) != "$" && prop != "Uuid") {
            buff2 += Blockly.Yail.JAVA_2TAB + comp2.$Name + "." + prop + "( \"" + comp2[prop] + "\" );" + Blockly.Yail.JAVA_NEW_LINE;
          }
        }
        //buff2 += Blockly.Yail.JAVA_NEW_LINE;  
      }
  ...
  buff3 = "";
  var track_multiples = " ";
  for (var i=0, block; block = tops[i]; i++) {
    var temp = "" + block;
    if (temp.indexOf("Initialize") > 0) {
      var descendants = block.getDescendants();
      for (var x=1,d; d = descendants[x];x++) {
        var temp2 = "" + d;
        temp2 = temp2.split(" ");
        if (temp2[0] == 'set') {
          buff3 += Blockly.Yail.JAVA_TAB + Blockly.Yail.JAVA_TAB;
          //handle set with '.' or set with '='
          //if value to set is string or not; case block will always be >
          if (temp2[3] == 'to') {
            if(temp2[4] == 'get') {
              buff3 += temp2[2] + " = " + temp2[6] + ";\n";
            } else if(temp2[4] == 'call') {
              if(temp2.length > 11){
                buff3 += temp2[2]+" = "+temp2[6]+temp2[7]+"("+"\""+temp2[10]+"\""+","+temp2[13]+");\n";
              } else {
                buff3 += temp2[2]+" = "+temp2[6]+temp2[7]+"("+temp2[9]+","+temp2[11]+");\n";
              }  
            } else {
              if (temp2.length > 4) {
                buff3 += temp2[2]+temp2[3]+temp2[4]+"("+temp2[7]+temp2[8]+temp2[9]+");\n";            
              } else {
                buff3 += temp2[2]+temp2[3]+temp2[4]+"("+temp2[7]+");\n";
              }     
            }
          } else {
            //if value being set is string or not; case block will always be 2 > in size.
            if (temp2[7] == 'get') {
              buff3 += temp2[2]+temp2[3]+temp2[4]+"("+temp2[9]+");\n";
            } else if (temp2.length > 8) {
              buff3 += temp2[2]+temp2[3]+temp2[4]+"("+"\""+temp2[8]+"\""+");\n";            
            } else {
              buff3 += temp2[2]+temp2[3]+temp2[4]+"("+temp2[7]+");\n";
            }          
          }
      ...
      if (temp.startsWith("when")) {
        ...
        var first = temp3[2].replace(/[0-9]/g, '');
        var second = temp3[3].replace(/\./g, '');
        if (track_multiples.indexOf(first) < 0){
          buff3 += Blockly.Yail.JAVA_ED + first + "\", \"" + second + "\");\n";
          track_multiples += first;
        }
      }
    }
  }
  
  return buff1+buff2+buff3+Blockly.Yail.JAVA_TAB+"}";
};

Blockly.Yail.writeDispatchEvent = function (tops) {
  var buff = "";
  var bufferino = "";
  var ifstatement = false;
  for (var x = 1, block; block = tops[x]; x++) {
    var temp1 = "" + block;
    if ((block.type != "global_declaration") && (temp1.indexOf("Initialize") < 0)) {
      var split_block = temp1.split(" ");
      var stripped_dot = split_block[3].replace(/\./g, '');
      var descendants = block.getDescendants();
      buff += Blockly.Yail.JAVA_NEW_LINE
      buff += Blockly.Yail.JAVA_COMP_IF + split_block[2] + Blockly.Yail.JAVA_EVENT_IF + stripped_dot + Blockly.Yail.JAVA_IF_END;
      for (var y=1, d; d = descendants[y];y++) {
        bufferino += "Me: " + d + "\n";
        var my_child = d.getChild();
        var my_parent = d.getParent();
        bufferino += "Parent: " + my_parent + "\n";
        bufferino += "Child: " + my_child + "\n";
        var temp2 = "" + d;
        temp2 = temp2.split(" ");
        var p_string = "" + my_parent;
        p_string = p_string.split(" ");
        if(temp2[0] == 'set' || temp2[0] == 'call'){
          buff += "\n\t\t\t\t";
          var pcount = 0;
          if(p_string[0] != 'if' && p_string[0] != 'else'){
            for(var tempy in temp2){
              var curr = temp2[tempy];
              if (curr == 'set' || curr == 'call' || curr == 'get' || curr == 'global'){
              //do nothing
              }
              else if (curr == 'to' || curr == 'tag'){
                buff += '(';
                pcount = pcount + 1;
              }
              else if (curr == '.Start'){
              ...
              else if (curr == 'valueIfTagNotThere' || curr == 'valueToStore'){
                buff += ',';
              }
              else {
                buff += temp2[tempy];
          ...
          if(temp2[0] == 'if' || temp2[0] == 'else'){
            var child = d.getChild();
            while (child){
              var cs = "" + child;
              bufferino += cs + "\n";
              //do all the good stuff until we got no children
...
Blockly.Yail.getComponentPropertiesLines = function(formName, componentJson, parentName, 
  ...
  //code.push(Blockly.Yail.JAVA_ADD_COMPONENT + componentType + Blockly.Yail.YAIL_SPACER 
    //+ componentName + Blockly.Yail.JAVA_SEMI + Blockly.Yail.JAVA_NEW_LINE);
...
Blockly.Yail.getFormPropertiesLines = function(formName, componentJson, includeComments) {
  var code = [];
  if (includeComments) {
    //code.push(Blockly.Yail.YAIL_COMMENT_MAJORt + formName + " extends Form implements HandlesEventDispatching {" + Blockly.Yail.YAIL_LINE_FEED);
  }
...
Blockly.Yail.getPropertySetterString = function(componentName, componentType, propertyName, 
  ...
  /*var code = Blockly.Yail.YAIL_SET_AND_COERCE_PROPERTY + Blockly.Yail.YAIL_QUOTE + 
    ...
    Blockly.Yail.YAIL_SPACER;*/
  var code = Blockly.Yail.JAVA_SET_PROPERTY + Blockly.Yail.JAVA_NEW_LINE + Blockly.Yail.JAVA_OPEN_BRACKET 
    + Blockly.Yail.JAVA_NEW_LINE + Blockly.Yail.JAVA_NEW_LINE;
  code = code.concat(componentName + " = new " + componentType + "(this);" + Blockly.Yail.JAVA_NEW_LINE 
    + Blockly.Yail.JAVA_NEW_LINE);
